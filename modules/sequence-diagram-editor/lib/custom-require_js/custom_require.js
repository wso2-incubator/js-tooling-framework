...
//      module has been loaded.
    ...
    function customRequire(identifier, callback, compiler) {
        if (identifier instanceof Array) {
            var modules = new Array();
            var modcount = identifier.length;
            for (var index = 0; index < identifier.length; index++) {
                (function(id, i) {
                    modules.push(customRequire(id, callback&&function(mod) {
                                                   modules[i] = mod;
...
// INFO Exporting require to global scope
    ...
    if (self.customRequire !== undefined)
        throw new SmoothieError('\'customRequire\' already defined in global scope');

    try {
        Object.defineProperty(self, 'customRequire', {'value':customRequire});
        Object.defineProperty(self.customRequire, 'resolve', {'value':resolve});
        Object.defineProperty(self.customRequire, 'path', {'get':function(){return requirePath.slice(0);}});
    }
    catch (e) {
        // NOTE IE8 can't use defineProperty on non-DOM objects, so we have to fall
        //      back to unsave property assignments in this case.
        self.customRequire = customRequire;
        self.customRequire.resolve = resolve;
        self.customRequire.path = requirePath.slice(0);
    }
...
function /*load*/(module/*, cache, pwd, source*/) {
    ...
    if (typeof module.id !== 'string')
        for (id in module)
            arguments[1]['$'+customRequire.resolve(id).id] = module[id].toString();
    arguments[2].shift();
}
